...
#define __SCIF_H__
#ifdef __KERNEL__
#error Kernelspace must use the scif.h provided by the kernel modules
// DO NOT EDIT THIS FILE; instead, edit the kernel modules' version and
// copy it here--preserving this preprocessor conditional, of course.
#endif
#ifndef _WIN32
...
#include <poll.h>
#endif /* __KERNEL__ */
#define MICACCESSAPI /* hack for Microsoft Windows* compatibility */
#else /* _WIN32 */
...
#define MICACCESSAPI __declspec(dllexport)
#else
#define MICACCESSAPI __declspec(dllimport)
#endif
#endif

#ifdef _OFF_T_DEFINED
#define off_t	UNDEFINED_OFF_T
#define _off_t	UNDEFINED__OFF_T
#else
...
#else
#define SCIF_POLLIN		0x01
#define SCIF_POLLOUT		0x04
#define SCIF_POLLERR		0x08
#define SCIF_POLLHUP		0x10
#define SCIF_POLLNVAL		0x20
#endif
 ...
 * Ports below SCIF_PORT_RSVD may be used for internal MPSS mechanisms
 * and should not be used by users.
 *
 *\return
 * Upon successful completion, scif_bind() returns the port number to which epd
 * is bound; otherwise -1 is returned and errno is set to indicate the error.
 *
 *\par Errors:
 *- EBADF
 ...
 *- EINVAL
 * - epd is not a valid endpoint descriptor, or
 * - The endpoint is not bound to a port, or
 ...
 *
 *\return
 * Upon successful completion, scif_close() returns 0; otherwise
 * -1 is returned and errno is set to indicate the error.
 *
 *\par Errors:
 *- EBADF
 ...
 * received; otherwise is returned and errno is set to indicate the error.
 *
 *\par Errors:
 *- EAGAIN
 ...
 *
 *\return
 * Upon successful completion, scif_unregister() returns 0; otherwise
 * -1 is returned and errno is set to indicate the error. In the event of an
 ...
 * for len bytes. On Microsoft Windows* scif_mmap() with SCIF_MAP_FIXED will
 * fail if any page in specified range is already reserved. Microsoft Windows*
 * does not allow to overlap memory regions.
 *
 ...
 *- ENOMEM
 * - Insufficient memory was available.
 * - Address is already in use (Microsoft Windows*).
 *- EINVAL
 * - epd is not a valid endpoint descriptor, or
 * - map_flags is invalid, or
 * - prot_flags is invalid , or
 ...
 *- ENXIO
 * - Addresses in the range [offset,offset+len-1] are invalid for the
 * registered address space of the peer of epd, or
 * - offset is negative
 *
 ...
 *   registered address space of epd.
 *- EPERM
 * - used memory window is not valid
 */
MICACCESSAPI
 ...
 * online nodes in the SCIF network including 'self'; otherwise
 * -1 is returned and errno is set to indicate the error.
 *
 *\par Errors:
 *- EFAULT
 ...
 * descriptors have been selected. Otherwise -1 is returned and
