...
int scif_reserve_dma_chan(struct scif_endpt *ep)
{
	...
	ep->rma_info.signal_pool = sdev->signal_pool;
...
static bool scif_is_usecache_set(int flags)
{
		   ...
		   off_t *out_offset, struct scif_window **out_window)
{
	...
	err = __scif_pin_pages((void *)(addr & PAGE_MASK),
			       aligned_len, &prot, 0, &pinned_pages, NULL);
	if (err)
		return err;
...
#else
		if (dma_sync_wait(chan, cookie) == DMA_SUCCESS) {
#endif
			err = 0;
		} else {
			err = -EIO;
...
static int scif_async_dma(struct scif_hw_dev *sdev, struct dma_chan *chan)
{
	...
	dma_async_issue_pending(chan);
retry:
	err = wait_for_completion_timeout(&done_wait, SCIF_DMA_TIMEOUT);
	if (!err) {
		err = -EIO;
		dev_err(&sdev->dev, "%s %d err %d\n",
			__func__, __LINE__, err);
			 ...
			 enum scif_rma_dir dir, bool last_chunk)
{
	...
	cache = scif_is_usecache_set(flags);
