...
static void scif_prog_signal_cb(void *arg)
{
	...
	dma_pool_free(status->ep->rma_info.signal_pool, status,
		      status->src_dma_addr);
}
...
static int _scif_prog_signal(scif_epd_t epd, dma_addr_t dst, u64 val)
{
	...
	struct dma_pool *signal_pool = ep->rma_info.signal_pool;
...
#ifdef CONFIG_INTEL_MIC_X100_DMA
	if (x100) {
#if LINUX_VERSION_CODE > KERNEL_VERSION(4, 2, 0)
		/*
...
#else
		tx = mic_dma_prep_status_lock(chan, dst, val, 0);
#endif
	}
#endif 	// CONFIG_INTEL_MIC_X100_DMA
	if (!x100) {
		status = dma_pool_alloc(signal_pool, GFP_KERNEL, &src);
		if (!status) {
			err = -ENOMEM;
			dev_err(&ep->remote_dev->sdev->dev, "%s %d err %d\n",
				__func__, __LINE__, err);
...
dma_fail:
	if (!x100)
		dma_pool_free(signal_pool, status, status->src_dma_addr);
alloc_fail:
	return err;
}
...
int _scif_fence_mark(scif_epd_t epd, int *mark)
{
	...
	if (x100) {
		tx = ddev->device_prep_dma_memcpy(chan, 0, 0, 0,
						  DMA_PREP_FENCE);
		if (!tx) {
			err = -ENOMEM;
			dev_err(&ep->remote_dev->sdev->dev, "%s %d err %d\n",
	...
	*mark = cookie = scif_dma_submit(tx);
	if (dma_submit_error(cookie)) {
		err = (int)cookie;
		dev_err(&ep->remote_dev->sdev->dev, "%s %d err %d\n",
			__func__, __LINE__, err);
